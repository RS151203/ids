...
class MainWindow(Ui_MainWindow, QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.comboBox_23.currentIndexChanged.connect(
            lambda: home_db_fun.Combobox_startup.show_recorrencia_options(self))
        self.comboBox_23.currentIndexChanged.connect(
            lambda: home_db_fun.Combobox_startup.show_recorrencia_options(self))
        self.comboBox_26.currentIndexChanged.connect(
            lambda: home_db_fun.Combobox_startup.show_set_dia_recorrencia(self))
    ...
    def campo_de_verificacao(self):
        ...
        if usuario + senha != '':
            ...
            if (os.path.exists('' + a + '/bando_de_valores.db')):
                ...
                pyautogui.confirm(
                    text='!!ATENÇÃO!!\nBanco de dados nao foi localizado, Por favor Criar arquivo em MENU>CRIAR BANCO DE DADOS',
                    title='BANCO DE DADOS NAO LOCALIZADO', buttons=['OK', 'Cancel'])
    ...
    def update(self):
        resposta = requests.get(
            'https://raw.githubusercontent.com/xjhowxjhow/HomeAplication1.0/main/version/version.txt')
...
def show_message():
    ...
    try:
        ...
        with open('HomeAppold.exe', 'wb') as novo_arquivo:
            ...
            msg.setText(
                "Nova atualização da aplicação foi encontrada, Clique em sair no icone na barra de tarefas para Sair da aplicação E Aplicar nova versao")
